 ...
 * 
 * Displays estimate data for public access via token.
...
export default function PublicView() {
  ...
  const loadEstimate = async (linkToken: string) => {
    try {
      const res = await projectsApi.getPublicView(linkToken)
      if (res.data.requiresPassword) {
        setRequiresPassword(true)
        setEstimateTitle(res.data.title)
        setViewName(res.data.viewName || '')
      } else {
        setData(res.data)
      }
    } catch {
  ...
  const handlePasswordSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await projectsApi.verifyPublicView(token, password.trim())
      setData(res.data)
        ...
        <div className="card max-w-md w-full">
          <div className="text-center mb-6">
            ...
            <p className="text-slate-400 text-sm">
              ...
              {viewName && <span className="block text-slate-500 text-xs mb-1">{viewName}</span>}
  ...
  return (
    <div className="min-h-screen bg-slate-900">
      ...
      <header className="border-b border-slate-700/50 bg-slate-900/95 sticky top-0 z-10 no-print">
        <div className="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8 py-3 sm:py-4 flex justify-between items-center">
          <div>
            ...
            {data.viewName && (
              <p className="text-xs sm:text-sm text-primary-400">{data.viewName}</p>
            )}
          </div>
          <button
      ...
      <div className="hidden print:block p-8 text-center border-b">
        ...
        {data.viewName && <p className="text-gray-500">{data.viewName}</p>}
      </div>
